...
func main() {
	...
	for k := 0; k < totalTests; k++ {
		...
		for {
			if single == nil {
				single = testbed.NewSingleWL(*wl, nParts, isPartition, nWorkers, curContention, *tp, float64(curCR), tmpTlen, tmpRR, tmpMP, curPS, testbed.PARTITION, double, partAlign)
			...
			oneTest(single, coord, ft, nWorkers, tm)
...
func oneTest(single *testbed.SingelWorkload, coord *testbed.Coordinator, ft [][]*testbed.Feature, nWorkers int, tm int) {
	for a := 0; a < 3; a++ {
